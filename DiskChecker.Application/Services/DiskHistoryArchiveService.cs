   ...
   public async Task<int> ImportDriveHistoryArchiveAsync(string zipPath, CancellationToken cancellationToken = default)
   {
      ...
      foreach(var archivedTest in payload.Drive.Tests)
      {
         ...
         if(exists)
         {
          ...
          var testRecord = new TestRecord
          {
             ...
             Grade = archivedTest.Grade.ToString(),
             Score = (int)archivedTest.Score,
             SurfaceProfile = archivedTest.SurfaceProfile?.ToString(),
             SurfaceOperation = archivedTest.SurfaceOperation?.ToString(),
             SurfaceTechnology = archivedTest.SurfaceTechnology?.ToString(),
             SecureErasePerformed = archivedTest.SecureErasePerformed ?? false,
    ...
    private static ArchiveTest ToArchiveTest(TestRecord test)
    {
       return new ArchiveTest
       {
          ...
          Score = test.Score,
          SurfaceProfile = string.IsNullOrEmpty(test.SurfaceProfile) ? null : Enum.Parse<SurfaceTestProfile>(test.SurfaceProfile),
          SurfaceOperation = string.IsNullOrEmpty(test.SurfaceOperation) ? null : Enum.Parse<SurfaceTestOperation>(test.SurfaceOperation),
          SurfaceTechnology = string.IsNullOrEmpty(test.SurfaceTechnology) ? null : Enum.Parse<DriveTechnology>(test.SurfaceTechnology),
